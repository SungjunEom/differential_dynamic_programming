...
\begin{algorithmic}[1]
\Require{Horizon \(N\), System $f(\cdot)$, Initial state $x_0$, Final state $x_N$ ,Loss function $l(\cdot)$, Final cost function $\phi(\cdot)$, Restriction $\Delta u_{max}$, Step length $\gamma(i)$}
...
\State $\delta u_{j+1} \leftarrow k_j+K_j\delta x_{j+1}$\textcolor{gray}{\Comment{updating the control input at j}}
