...
class System:
    ...
    def forward(self):
        ...
        for j in range(50):  # Maximum gamma iteration
            for i in range(len(self.inputs) - 1):
                ...
                dcontrol_i = self.ks[i + 1] * pow(1 / 2, j) + self.Ks[i + 1] * (
                    x_hat[i + 1] - self.states[i + 1]
                )
    ...
    def backward(self):
        ...
        for i in range(len(self.states) - 2, -1, -1):
            ...
            Qx = self.dloss("x", self.states[i], self.inputs[i]) + dx * Vx
            Qu = self.dloss("u", self.states[i], self.inputs[i]) + du * Vx
            Qxx = (
                self.dloss("xx", self.states[i], self.inputs[i])
                + dx**2 * Vxx
                + Vx * self.dsys("xx", self.states[i], self.inputs[i])
            )
            Qux = (
                self.dloss("ux", self.states[i], self.inputs[i])
                + du * Vxx * dx
                + Vx * self.dsys("ux", self.states[i], self.inputs[i])
            )
            Quu = (
                self.dloss("uu", self.states[i], self.inputs[i])
                + du**2 * Vxx
